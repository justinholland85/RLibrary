...
Lib.HeatLegend2  <-  function(Names, Cols, X0, X1, Y0, Y1, Move.X, Move.Y,
                             Eps, Cex, TextCol, Border){
  ...
  X.A  <-  X0 + X.dA * Move.X
  X.B  <-  X0 + X.dB * Move.X + (X1 - X0) * (1 - Move.X)
  
  Y.dA  <-  seq(0, N-1) * (Y1 - Y0) / N
  Y.dB  <-  seq(1, N)   * (Y1 - Y0) / N 
  
  Y.A  <-  Y0 + Y.dA * Move.Y
  Y.B  <-  Y0 + Y.dB * Move.Y + (Y1 - Y0) * (1 - Move.Y)
  ...
  for(i in 1:N){
    ...
    polygon(X, Y, col="white", border=Border)
    polygon(X, Y, col=Cols[i], border=Border)
